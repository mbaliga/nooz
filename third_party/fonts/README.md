...
| The "Nooz" wordmark only | **Playfair Display Black** | `playfair_display_black.ttf` | Google Fonts, unmodified |
...
owner's actual splash mock (2026-07 SVG/PNG drop) sets "Nooz" in a
high-contrast Didone face — thin hairline diagonals, flat slab serifs, round
geometric bowls — structurally different from Hyle Print's moderate-contrast
Literata chassis. Playfair Display Black is the closest freely-licensed match;
the mock's own wordmark is vector-outlined with no embedded font name, so the
exact commercial face (if any) couldn't be extracted from it.
...
derivative works; Playfair Display is the unmodified upstream Google Fonts
release); the per-family provenance notes are `Hyle*-LICENSE-NOTE.txt` /
`PlayfairDisplay-LICENSE-NOTE.txt` and the license body is `OFL-1.1.txt`.
